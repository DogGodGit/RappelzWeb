...
public partial class Setting_SharingEvents_SharingEventSenderLog : System.Web.UI.Page
{
    ...
    private void PageLoad()
    {
       ...
       // DBUtil.CloseDBConn(conn);
        ...
        ////엑셀다운로드 페이지
        m_sPopupUrl = String.Format("/Setting/Popup/Popup_SharingEventSenderLog.aspx", "");
        ...
        // 연결정보 가져오기 (DB커넥션 포함됨)
       // string sConnectionString = DataUtil.GetGameServerConnectionString(m_nGameServerId);

        // DB연결
       // SqlConnection connGame = DBUtil.GetGameDBConn(sConnectionString);

        // 공유하기 - 발신자 로그
        DataTable senderLog = DacUser.SharingEventSenderLogs(conn, null, m_nGameServerId, m_sName, m_nType, m_nRowsPerPage, m_nPage, out m_nTotalCount);
    ...
    protected void WBtnSearch_OnClick(object sender, EventArgs e)
    {
        ...
        {
            ComUtil.MsgBox("서버를 선택해주세요.", "history.back();"); //서버를 선택해 주세요
            return;
        }

        Response.Redirect("/Setting/SharingEvents/SharingEventSenderLog.aspx?SVR=" + WDDLServer.SelectedValue + "&NM=" + WTxtSearch.Text.Trim() + "&TP="+ WDDLType.SelectedValue);
